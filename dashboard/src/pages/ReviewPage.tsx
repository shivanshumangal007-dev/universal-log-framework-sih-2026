...
function parseFields(fieldsJson: string): EditableField[] {
	try {
...
function fieldsToPayload(fields: EditableField[]): CorrectedFields {
...
export default function ReviewPage() {
	...
	const filtered = useMemo(() => {
		...
		return items.filter(
			(i) =>
				i.raw_line.toLowerCase().includes(q) || i.source.toLowerCase().includes(q),
		);
	}, [items, search]);
	...
	const toggleExpand = (id: string) => {
		setExpanded((prev) => {
			...
			if (next.has(id)) {
				...
				const item = items.find((reviewItem) => reviewItem.event_id === id);
				if (item && !editedFields[id]) {
					setEditedFields((current) => ({
						...current,
						[id]: parseFields(item.fields_json),
					}));
				}
				next.add(id);
			}
	...
	const updateField = (
		id: string,
		index: number,
		property: keyof EditableField,
		value: string,
	) => {
		setEditedFields((current) => ({
			...current,
			[id]: (current[id] || []).map((field, fieldIndex) =>
				fieldIndex === index ? { ...field, [property]: value } : field,
			),
		}));
		setChangedFields((current) => new Set(current).add(id));
	};
	...
	const handleApprove = async (id: string) => {
		...
		try {
			await api.approveReview(
				id,
				changedFields.has(id) ? fieldsToPayload(editedFields[id] || []) : undefined,
			);
														...
														<div className='space-y-3 text-xs'>
															...
															<div>
																...
																<div className='space-y-2 rounded border border-neutral-200 bg-white p-3'>
																	{(
																		editedFields[item.event_id] || parseFields(item.fields_json)
																	).map((field, index) => (
																		<label
																		...
																		>
																			<input
																				...
																				className='w-32 shrink-0 rounded border border-neutral-200 px-2 py-1 font-mono text-neutral-700 outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500'
																			...
																			<input
																				...
																				className='min-w-0 flex-1 rounded border border-neutral-200 px-2 py-1 font-mono text-neutral-700 outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500'
																			/>
																		</label>
																	))}
																</div>
